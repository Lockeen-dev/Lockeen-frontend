...
const Icon = ({ d, fill, size = 20, sw = 1.8, children, viewBox = "0 0 24 24", style, color }) => (
...
const Pencil      = (p) => <Icon {...p}><path d="M17 3a2.85 2.83 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"/></Icon>;
const Trash2      = (p) => <Icon {...p}><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></Icon>;
...
const LockeenLogo = ({ size = 20, muted = false }) => (
  <img
...
const XMark       = (p) => <Icon {...p}><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></Icon>;
...
const SidebarPanel = (p) => <Icon {...p} viewBox="0 0 24 24"><rect x="5" y="4" width="14" height="16" rx="3"/><path d="M9 8v8"/></Icon>;
...
const RefreshCw    = (p) => <Icon {...p}><path d="M21 12a9 9 0 0 1-9 9 9.75 9.75 0 0 1-6.74-2.74L3 16"/><path d="M3 21v-5h5"/><path d="M3 12a9 9 0 0 1 9-9 9.75 9.75 0 0 1 6.74 2.74L21 8"/><path d="M21 3v5h-5"/></Icon>;
...
const Stop     = (p) => <Icon {...p}><rect x="3" y="3" width="18" height="18" rx="2"/></Icon>;
const GripDots = (p) => <Icon {...p} viewBox="0 0 12 12" sw={0} fill="currentColor"><circle cx="3" cy="3" r="1.2"/><circle cx="6" cy="3" r="1.2"/><circle cx="9" cy="3" r="1.2"/><circle cx="3" cy="6" r="1.2"/><circle cx="6" cy="6" r="1.2"/><circle cx="9" cy="6" r="1.2"/><circle cx="3" cy="9" r="1.2"/><circle cx="6" cy="9" r="1.2"/><circle cx="9" cy="9" r="1.2"/></Icon>;
const Coins    = (p) => <Icon {...p}><circle cx="8" cy="8" r="6"/><path d="M18.09 10.37A6 6 0 1 1 10.34 18"/><path d="M7 6h1v4"/><line x1="16.71" y1="13.88" x2="13.83" y2="17.29"/></Icon>;
...
const UI_T = {
  en: { home:'Home', exams:'Exams', flash:'Flash', flashcards:'Flashcards', quiz:'Quiz', stats:'Stats', dashboard:'Dashboard', myExams:'My Exams', calendar:'Calendar', aiTutor:'AI Tutor', analytics:'Analytics', goodMorning:'Good morning', ready:'Ready to continue your learning journey?', todaySchedule:'Today schedule', dailyTasks:'Daily tasks', recommended:'Recommended today', openCalendar:'Open Calendar', examSub:'Your exams and study materials', newExam:'New Exam', studyPlan:'Study Plan', searchExams:'Search exams...', signOut:'Sign out' },
  it: { home:'Home', exams:'Esami', flash:'Flash', flashcards:'Flashcards', quiz:'Quiz', stats:'Stats', dashboard:'Dashboard', myExams:'I miei esami', calendar:'Calendario', aiTutor:'AI Tutor', analytics:'Analytics', goodMorning:'Buongiorno', ready:'Pronto a continuare il tuo percorso?', todaySchedule:'Programma di oggi', dailyTasks:'Task del giorno', recommended:'Consigliati oggi', openCalendar:'Apri Calendario', examSub:'I tuoi esami e materiali di studio', newExam:'Nuovo esame', studyPlan:'Piano studio', searchExams:'Cerca esami...', signOut:'Esci' },
  de: { home:'Home', exams:'Prüfungen', flash:'Flash', flashcards:'Karten', quiz:'Quiz', stats:'Stats', dashboard:'Dashboard', myExams:'Meine Prüfungen', calendar:'Kalender', aiTutor:'AI Tutor', analytics:'Analytics', goodMorning:'Guten Morgen', ready:'Bereit, weiterzulernen?', todaySchedule:'Heutiger Plan', dailyTasks:'Tagesaufgaben', recommended:'Heute empfohlen', openCalendar:'Kalender öffnen', examSub:'Deine Prüfungen und Lernmaterialien', newExam:'Neue Prüfung', studyPlan:'Lernplan', searchExams:'Prüfungen suchen...', signOut:'Abmelden' },
  es: { home:'Inicio', exams:'Exámenes', flash:'Flash', flashcards:'Tarjetas', quiz:'Quiz', stats:'Stats', dashboard:'Dashboard', myExams:'Mis exámenes', calendar:'Calendario', aiTutor:'Tutor IA', analytics:'Analíticas', goodMorning:'Buenos días', ready:'¿Listo para seguir aprendiendo?', todaySchedule:'Programa de hoy', dailyTasks:'Tareas del día', recommended:'Recomendado hoy', openCalendar:'Abrir calendario', examSub:'Tus exámenes y materiales de estudio', newExam:'Nuevo examen', studyPlan:'Plan de estudio', searchExams:'Buscar exámenes...', signOut:'Salir' },
  fr: { home:'Accueil', exams:'Examens', flash:'Flash', flashcards:'Cartes', quiz:'Quiz', stats:'Stats', dashboard:'Dashboard', myExams:'Mes examens', calendar:'Calendrier', aiTutor:'Tuteur IA', analytics:'Analytics', goodMorning:'Bonjour', ready:'Prêt à continuer ton apprentissage ?', todaySchedule:'Planning du jour', dailyTasks:'Tâches du jour', recommended:'Recommandé aujourd’hui', openCalendar:'Ouvrir le calendrier', examSub:'Tes examens et supports de révision', newExam:'Nouvel examen', studyPlan:'Plan de révision', searchExams:'Rechercher des examens...', signOut:'Déconnexion' },
  pt: { home:'Início', exams:'Exames', flash:'Flash', flashcards:'Flashcards', quiz:'Quiz', stats:'Stats', dashboard:'Dashboard', myExams:'Meus exames', calendar:'Calendário', aiTutor:'Tutor IA', analytics:'Analytics', goodMorning:'Bom dia', ready:'Pronto para continuar a aprender?', todaySchedule:'Agenda de hoje', dailyTasks:'Tarefas do dia', recommended:'Recomendado hoje', openCalendar:'Abrir calendário', examSub:'Os teus exames e materiais de estudo', newExam:'Novo exame', studyPlan:'Plano de estudo', searchExams:'Procurar exames...', signOut:'Sair' },
};
function tt(lang, key) { return (UI_T[lang] && UI_T[lang][key]) || UI_T.en[key] || key; }
...
/* ===================== AUTH SCREEN ===================== */
function AuthModal({ initialMode = "signin", onAuth, onClose, darkMode }) {
...
/* ===================== STUDY TIMER ===================== */
function StudyTimer({ onSessionSaved, startTrigger }) {
  const PS = 1500;
  const PB = 300;
  const CIRC = 376.99;
...
/* ===================== SIDEBAR ===================== */
function Sidebar({ tab, setTab, lang = 'en', collapsed = false, onToggleCollapsed }) {
  const items = [
        ...
        {items.map(({ id, label, Icon: I }) => {
          ...
          return (
            <button
              ...
              style={{ ...sideS.item, ...(collapsed ? sideS.itemCollapsed : null), ...(active ? sideS.itemActive : null), ...(active && collapsed ? sideS.itemCollapsedActive : null) }}
            >
              <span style={{ display: 'inline-flex', alignItems: 'center', color: active ? '#ffffff' : '#6B7280' }}>
                <I size={18} muted={!active} />
              </span>
              {!collapsed && <span style={{ color: active ? '#ffffff' : '#6B7280' }}>{label}</span>}
            </button>
          );
        })}
        </nav>
...
const sideS = {
  ...
  itemCollapsedActive: { boxShadow: '0 8px 22px -10px rgba(55,48,232,.65), 0 0 0 3px rgba(55,48,232,.12)' },
...
function inferSubjectFromName(value) {
...
const PALETTE_CYCLE = ['#EEF2FF', '#FDF2F8', '#ECFEFF', '#FEF3C7', '#DBEAFE', '#EDE9FE'];
const DOT_CYCLE     = ['var(--indigo)', 'var(--purple)', '#06B6D4', '#F59E0B', '#3B82F6', '#7C3AED'];

const cellularRespirationCards = [
...
const cellularRespirationQuestions = [
  {
...
function makeSampleChapter(id, title, updated, pages, files, idx, cards = [], questions = []) {
